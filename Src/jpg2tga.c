...
// subsampling factor).
uint8 *pjpeg_load_from_file(uint8_t *pFilename, int size, int *x, int *y, int *comps, pjpeg_scan_type_t *pScan_type, int reduce)
{
   ...
   *comps = 0;
   ...
   g_pInFile = fmemopen(pFilename, size, "r");//fopen(pFilename, "rb");
   if (!g_pInFile)
      return NULL;
